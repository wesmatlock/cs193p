    ...
    ZStack(alignment: .leading) {
      ...
      if !isEditing {
        Text(text)
          ...
          .onAppear {
            // any time we move from editable to non-editable we want to report any changes that happened to
            // the text while were editable (i.e. we never "abandon" changes)
            self.callOnChangedIfChanged()
          }
      }
    }
